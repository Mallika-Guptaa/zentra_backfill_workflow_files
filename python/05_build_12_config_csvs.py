...
# ---------------------------------------------------------------------
# EXACT DAILY 12-CONFIG UPDATE
# ---------------------------------------------------------------------

def _parse_daily_remote_paths(summary: dict, raw_prefix: str) -> list[str]:
    """
    Read ONLY the explicit raw CSV paths produced by the latest daily fetch.
    We intentionally do not scan the historical S3 raw inventory here.
    """
    raw_prefix = raw_prefix.strip().rstrip("/")
    paths = []
    ...
    if isinstance(serials, dict):
        for _, info in serials.items():
            ...
            if remote_path:
                path = str(remote_path).strip().lstrip("/")
                if path.startswith(raw_prefix + "/") and path.lower().endswith(".csv"):
                    paths.append(path)

    return sorted(set(paths))


def _source_record_for_path(mapping: pd.DataFrame, source_path: str) -> dict:
    serial = _extract_serial_from_source_path(source_path)
    return {
        **_build_source_record(mapping=mapping, serial=serial),
        "source_file": source_path,
    }


def _local_months_by_config(
    mapped_df: pd.DataFrame,
    timezone_name: str = "America/Los_Angeles",
) -> dict[str, list[str]]:
    """
    Return affected YYYY-MM months for each configuration using Oregon local time.
    """
    ...
    work = mapped_df.copy()
    dt = pd.to_datetime(work.get("datetime"), errors="coerce", utc=True)
    work["_local_dt"] = dt.dt.tz_convert(timezone_name)
    work["_month_key"] = work["_local_dt"].dt.strftime("%Y-%m")
    work = work.dropna(subset=["configuration_code", "_month_key"])

    for config_code, group in work.groupby("configuration_code"):
        result[str(config_code)] = sorted(
            group["_month_key"].dropna().astype(str).unique().tolist()
        )
    return result


def update_12_configs_from_daily_summary(
    raw_prefix: str = "zentra_raw_backfill",
    daily_state_prefix: str = "zentra_daily_update_state",
    daily_summary_file: str = "daily_update_summary_latest.json",
    output_prefix: str = "zentra_final_12_configs",
    manifest_prefix: str = "zentra_processing_state",
    manifest_file: str = "processed_raw_manifest.json",
    update_summary_file: str = "daily_12config_update_summary_latest.json",
    timezone_name: str = "America/Los_Angeles",
):
    """
    Fast daily updater.

    It does NOT scan all historical raw files. Instead it:
      1. reads daily_update_summary_latest.json
      2. takes only the exact raw CSV paths created by that daily fetch
      3. skips paths already committed in processed_raw_manifest.json
      4. exact-maps rows using logger_serial_number + port_num
      5. merges only affected final config CSVs
      6. commits the manifest only after all affected outputs upload successfully

    Rerun safety:
      If a previous attempt uploaded some config files but failed before manifest
      commit, this function removes rows from the same source_file before
      re-appending them, so retrying is idempotent.
    """
    raw_prefix = raw_prefix.strip().rstrip("/")
    daily_state_prefix = daily_state_prefix.strip().rstrip("/")
    output_prefix = output_prefix.strip().rstrip("/")
    manifest_prefix = manifest_prefix.strip().rstrip("/")
    timezone_name = str(timezone_name).strip()

    if not _s3_object_exists(daily_state_prefix, daily_summary_file):
        raise RuntimeError(
            f"Missing daily fetch summary: "
            f"{daily_state_prefix}/{daily_summary_file}"
        )

    daily_summary = _load_processing_manifest(
        daily_state_prefix,
        daily_summary_file,
    )

    daily_paths = _parse_daily_remote_paths(
    ...
    if previous_processed and previous_version != MAPPING_VERSION:
        raise RuntimeError(
            "Processing manifest mapping version differs from current mapping. "
            "Run the full 12-config rebuild first, then resume daily updates."
        )

    new_paths = [
        path for path in daily_paths
        if path not in previous_processed
    ]
    ...
    summary = {
        "created_at_utc": now_utc,
        "mapping_version": MAPPING_VERSION,
        "daily_fetch_summary": f"{daily_state_prefix}/{daily_summary_file}",
        "daily_fetch_status": daily_summary.get("status", "unknown"),
        "failed_serials_in_fetch": daily_summary.get("failed_serials", []),
        "daily_paths_in_summary": daily_paths,
        "already_committed_paths": sorted(set(daily_paths) - set(new_paths)),
        "new_paths_to_process": new_paths,
        "processed_paths": [],
        "affected_configs": [],
        "affected_months_by_config": {},
        "rows_added_by_config": {},
        "status": "started",
    }

    # No new sources is a successful no-op.
    if not new_paths:
        summary["status"] = "no_new_sources"
        _put_json(summary, manifest_prefix, update_summary_file)
        _put_json(
            summary,
            manifest_prefix,
            f"daily_12config_update_summary_{run_stamp}.json",
        )
        faasr_log("Daily 12-config update: no uncommitted raw sources.")
        return

    prepared_pieces = []
    successfully_read_paths = []
    read_failures = []

    for idx, source_path in enumerate(new_paths):
        ...
        if not serial:
            read_failures.append({
                "source_file": source_path,
                "error": "Could not derive logger serial from path.",
            })
            continue
        ...
        if selected_mapping.empty:
            read_failures.append({
                "source_file": source_path,
                "error": f"No mapping rows found for serial {serial}.",
            })
            continue
        ...
        try:
            ...
            successfully_read_paths.append(source_path)
        except Exception as exc:
            read_failures.append({
                "source_file": source_path,
                "error": str(exc),
    ...
    if read_failures:
        summary["status"] = "failed_before_publish"
        summary["read_failures"] = read_failures
        _put_json(summary, manifest_prefix, update_summary_file)
        raise RuntimeError(
            "Daily 12-config update could not read every new raw source. "
            f"Failures: {read_failures}"
        )

    all_new_raw = (
        pd.concat(prepared_pieces, ignore_index=True)
        if prepared_pieces
        else pd.DataFrame()
    )

    if all_new_raw.empty:
        # Sources were readable but contained no mapped-port rows.
        candidate_manifest = dict(previous_manifest) if previous_manifest else {}
        candidate_processed = dict(previous_processed)
        for path in successfully_read_paths:
            candidate_processed[path] = {
                **_source_record_for_path(mapping, path),
                "processed_at_utc": now_utc,
                "mapping_version": MAPPING_VERSION,
            }
        ...
        _put_json(candidate_manifest, manifest_prefix, manifest_file)

        summary["processed_paths"] = successfully_read_paths
        summary["status"] = "success_no_mapped_rows"
        _put_json(summary, manifest_prefix, update_summary_file)
        _put_json(
            summary,
            manifest_prefix,
    ...
    # Prevent stale mapping metadata from entering the mapping merge.
    raw_drop_cols = [
        c for c in [
            "configuration_code",
            "location_code",
            ...
            "port_description",
            "notes",
        ] if c in all_new_raw.columns
    ]
    all_new_clean = all_new_raw.drop(columns=raw_drop_cols)
    ...
    # Port 5 from z6-19598 belongs to S_W_D only from 2025-08-21 local time.
    special_port5 = (
        (mapped["configuration_code"] == "S_W_D")
        & (mapped["logger_serial_number"] == "z6-19598")
        & (mapped["port_num"] == 5)
    )
    mapped_dt = pd.to_datetime(mapped["datetime"], errors="coerce", utc=True)
    port5_date_ok = mapped_dt.notna() & (mapped_dt >= PORT5_SWD_START_UTC)
    mapped = mapped[(~special_port5) | port5_date_ok].copy()
    mapped = _dedupe_and_sort(mapped)

    affected_configs = sorted(
        mapped["configuration_code"].dropna().astype(str).unique().tolist()
    )
    affected_months = _local_months_by_config(
        mapped,
        timezone_name=timezone_name,
    )

    source_set = set(successfully_read_paths)
    publish_frames: dict[str, pd.DataFrame] = {}

    # Build every affected output completely in local memory first.
    for config_code in affected_configs:
        filename = f"{config_code}.csv"
        ...
        if _s3_object_exists(output_prefix, filename):
            existing_df = _download_csv(
                output_prefix,
                filename,
                f"_daily_existing_{filename}",
            )

        generated_df = mapped[
            mapped["configuration_code"] == config_code
        ].copy()

        # Idempotency: remove these exact raw sources if a prior partial publish
        # occurred before the manifest was committed.
        if (
            existing_df is not None
            and not existing_df.empty
            and "source_file" in existing_df.columns
        ):
            existing_df = existing_df[
                ~existing_df["source_file"].astype(str).isin(source_set)
            ].copy()

        final_df = _merge_existing_and_generated(
            existing_df=existing_df,
            generated_df=generated_df,
        )
        publish_frames[config_code] = final_df

    # Publish only after all affected dataframes were successfully prepared.
    for config_code, final_df in publish_frames.items():
        filename = f"{config_code}.csv"
        ...
        summary["rows_added_by_config"][config_code] = int(
            len(mapped[mapped["configuration_code"] == config_code])
        )
    ...
    summary.update({
        "status": "success",
        "processed_paths": successfully_read_paths,
        "affected_configs": affected_configs,
        "affected_months_by_config": affected_months,
    })
    ...
    faasr_log(
        "Daily exact 12-config update complete. "
        f"sources={len(successfully_read_paths)}, "
        f"affected_configs={affected_configs}"
    )
